...
def pop_average(session_id, probe, stim):
    ...
    # separate by layer - Xiaoxuan code




def state_definition(session, probe, stim, trial_duration, state_var_type='power', phasic_event_data=None):
    ...
    state_variable_matrix_LFP = construct_state_variables(phasic_event_data, trial_duration, state_var_type)
    print('Designed state variable matrix')

    # state variables
    layers = np.argsort(phasic_event_data.ypos.unique())
    plt.figure()
    sns.heatmap(np.mean(state_variable_matrix_LFP[layers, :, 0, :], axis=1), vmin=0, vmax=0.4, cmap='Spectral_r')

    plt.figure()
    sns.heatmap(np.mean(state_variable_matrix_LFP[layers, :, 1, :], axis=1), vmin=0, vmax=0.4, cmap='Spectral_r')

    plt.figure()
    sns.heatmap(np.mean(state_variable_matrix_LFP[layers, :, 2, :], axis=1), vmin=0, vmax=0.4, cmap='Spectral_r')

    plt.figure()
    sns.heatmap(np.mean(state_variable_matrix_LFP[layers, :, 3, :], axis=1), vmin=0, vmax=0.4, cmap='Spectral_r')

    state_variable_matrix_nrn = pop_average(session, probe, stim)


    # behavior
    running_speed, rs_times, pupil_data, pd_times = get_velocity_and_pupil_area(session, stim, probe, state_variable_matrix.shape[3]/625)
    ...
    for n, bin in enumerate(win):
        ...
        # dat_frame['dim3'] = embedding_UMAP[:, 2]
        dat_frame['times'] = times
        dat_frame['trials'] = trials
        fig = plt.figure()
        # ax1 = fig.add_subplot(projection='3d')
        # ax1.scatter(dat_frame.dim1, dat_frame.dim2, dat_frame.dim3, c=dat_frame.times, cmap="BuPu", alpha=0.7)
        plt.scatter(dat_frame.dim1, dat_frame.dim2, c=dat_frame.times, cmap="BuPu", alpha=0.7)
        plt.title('UMAP projection over time')

        fig = plt.figure()
        # ax2 = fig.add_subplot(projection='3d')
        # ax2.scatter(dat_frame.dim1, dat_frame.dim2, dat_frame.dim3, c=dat_frame.trials, cmap="Blues", alpha=0.7)
        plt.scatter(dat_frame.dim1, dat_frame.dim2, c=dat_frame.trials, cmap="Blues", alpha=0.7)
        plt.title('UMAP projection over time, colored by trials')
        ...
        plt.subplot(414)
        state_behavior = np.zeros([3, mean_state_shifted_axis.shape[3]])
        state_behavior[0, :] = state_estimates.reshape(-1)
        state_behavior[1, :] = binned_running_speed
        state_estimates[2, :] = binned_pupil_size
...
state_definition(767871931, 'probeC', 'natural_movie_one_more_repeats', 30*625, 'clustering')
